...
function appendOrderRows(sheet, order, emailStatus) {
  ...
  const range = sheet.getRange(firstRow, 1, rows.length, rows[0].length);
  range.setValues(rows);
  frameOrder(sheet, firstRow, rows.length, rows[0].length);
}

/**
 * ממסגר את שורות ההזמנה במלבן.
 *
 * נבחר מלבן ולא צבע רקע במכוון: מספר הצבעים מוגבל, ואחרי עשרות הזמנות
 * צבעים חוזרים על עצמם ומאבדים את המשמעות. מסגרת עובדת לאינסוף הזמנות,
 * נשארת קריאה בהדפסה, ולא מתנגשת עם סינון או מיון.
 */
function frameOrder(sheet, firstRow, numRows, numCols) {
  const range = sheet.getRange(firstRow, 1, numRows, numCols);
...
function rebuildProductionSummary(sheets) {
  ...
  data.forEach(function (row) {
    const key = row.pickupDate + '|' + row.branch + '|' + row.product;
    if (!buckets[key]) {
      buckets[key] = {
        date: row.pickupDate,
        branch: row.branch,
  ...
  const rows = Object.keys(buckets)
    ...
    .sort(function (a, b) {
      if (a.branch !== b.branch) return String(a.branch).localeCompare(String(b.branch), 'he');
      return String(a.product).localeCompare(String(b.product), 'he');
    })
    .map(function (b) {
      return [b.date, b.branch, b.product, b.qty, Object.keys(b.references).length, b.value];
    });
...
function rebuildRevenueSummary(sheets) {
  ...
  data.forEach(function (row) {
    ...
    byDate[row.branch] = (byDate[row.branch] || 0) + row.lineTotal;
...
/* סימן המטבע משמאל למספר, כמו באתר */
...
function runTestOrder() {
  const order = {
    ...
    pickup: {
      date: '2026-09-11',
      dateLong: 'יום שישי, 11 בספטמבר 2026',
      branchId: 'kinneret',
