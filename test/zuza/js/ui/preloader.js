...
/** מחזור מלא — ארבע אותיות */
const CYCLE_MS = LETTERS.length * STEP_MS;
...
export function createBrandLoader({ caption = '', inline = false, forceFallback = false } = {}) {
   ...
   * המיקום ברצף מחושב מהזמן שחלף בפועל, ולא מספירת פעימות.
   * זה ההבדל בין מחוון שמדלג על אות כשהדפדפן עמוס, לבין מחוון שנתקע —
   * ובדיוק בזמן הטעינה הדפדפן עמוס.
   */
  function tick(now) {
    const index = Math.floor(((now - startedAt) / STEP_MS) % LETTERS.length);
