...
function media(product) {
  ...
  const img = el('img', {
     ...
     */
    /*
     * רוחב התמונה = רוחב העמודה ברשת: שלוש עמודות מ-62em, שתיים מ-34em,
     * אחת מתחת לזה. כך הדפדפן מוריד את הקובץ בגודל הנכון לכל מסך.
     */
    sizes: '(min-width: 62em) 300px, (min-width: 34em) 45vw, 92vw',
  });
...
export function initMenu({ onAdd, onOpenCart } = {}) {
   ...
   * פתיחה אוטומטית של המגירה הייתה מסתירה את שאר התפריט
   * בדיוק ברגע שבו הלקוח ממשיך לבחור.
   */
  function confirmChange(message) {
    toast(message, {
      action: { label: 'מעבר לסל', onClick: () => onOpenCart?.() },
    });
  }
  ...
  root.addEventListener('click', (event) => {
    ...
    switch (trigger.dataset.action) {
      case 'add':
        ...
        confirmChange(`${product.name} נוסף להזמנה`);
        onAdd?.(product);
        break;
      case 'inc':
        ...
        confirmChange(`${product.name} · ${qty + 1} יחידות`);
        break;
      case 'dec':
        cart.setQty(product.id, qty - 1);
        if (qty - 1 > 0) confirmChange(`${product.name} · ${qty - 1} יחידות`);
        break;
      default:
        break;
    }
