 ...
 *
 * כל תמונה נבנית בשלושה חיתוכים ולא רק בשלושה גדלים: 4:3 בטלפון,
 * 16:9 בטאבלט ו-21:9 במסכים רחבים — כל אחד קרוב לצורת התיבה שה-CSS
 * בפועל מציג בטווח הרוחבים שלו. החיתוך נעשה בצד Cloudinary עם g_auto,
 ...
 *
 * מעל 62em, ה-CSS מגביל את הגובה ל-540px בעוד הרוחב נשאר 100% מהחלון —
 * כלומר הצורה בפועל רחבה בהרבה מ-16:9 (עד פי 3–4 במסכים גדולים).
 * בלי חיתוך ייעודי, הדפדפן היה מקבל תמונה שכבר נחתכה ל-16:9 וחותך ממנה
 * עוד מלמעלה ומלמטה במרכז — בלי מודעות לתוכן, מה שקטע עוגות ופירות
 * שיושבים קרוב לשוליים. ar_21:9 כאן קרוב בהרבה לצורה האמיתית על המסך,
 * כך שהחיתוך החכם של Cloudinary (g_auto) הוא זה שקובע מה נשאר בפריים.
 */
const DESKTOP = { media: '(min-width: 62em)', ratio: 'ar_21:9', widths: [1600, 2200, 2880] };
...
function slide(image, index) {
  const picture = el('picture', {}, [
    el('source', {
      media: DESKTOP.media,
      sizes: '100vw',
      srcset: srcset(image.url, DESKTOP.ratio, DESKTOP.widths),
    }),
    el('source', {
      media: WIDE.media,
      sizes: '100vw',
